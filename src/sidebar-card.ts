...
async function build() {
  ...
  if (
    ...
    if (!appLayout.querySelector("#customSidebarWrapper")) {
      ...
      const contentContainer = appLayout.querySelector(
        "#view"
      ) as HTMLElement | null;
  ...
  if (headerConfig && headerConfig.enabled !== false) {
    ...
    if (!viewEl) {
      ...
      const sidebarWrapper = appLayout.querySelector(
        "#customSidebarWrapper"
      ) as HTMLElement | null;
      const targetContent = sidebarWrapper ?? viewEl;

      let headerWrapper = appLayout.querySelector(
        "#customHeaderWrapper"
      ) as HTMLElement | null;
      if (!headerWrapper) {
      ...
      let headerHost = headerWrapper.querySelector(
      ...
      function ensureFlipStage(headerHost: HTMLElement) {
        let stage = headerHost.querySelector(
          "#headerFlipStage"
        ) as HTMLElement | null;
        ...
        const headerCardEl = headerHost.querySelector(
          "header-card"
        ) as HTMLElement | null;
      ...
      function triggerHeaderFlip(
        ...
        const initialHostH = Math.max(
          72,
          Math.round(headerHost.getBoundingClientRect().height || 0)
        );
        ...
        const savedViewPaddingTopProp = viewEl.style.getPropertyValue(
          "padding-top"
        );

        const freezeLayout = () => {
          (window as any).__silvioFlipActive = true;
          headerWrapper.style.paddingTop = window.getComputedStyle(
            headerWrapper
          ).paddingTop;
        ...
        const showHaOverlay = (on: boolean) => {
          const restore = (window as any)[overlayKey] as
            | (() => void)
            | undefined;
        ...
        const animateToBack = async () => {
          ...
          await new Promise<void>(resolve => {
            ...
            front.animate([{ opacity: 1 }, { opacity: 0 }], {
              duration: rollMs,
              easing: "ease-in-out",
              fill: "forwards"
            });
        ...
        const animateToFront = async () => {
          ...
          await new Promise<void>(resolve => {
            ...
            back.animate([{ opacity: 1 }, { opacity: 0 }], {
              duration: rollMs,
              easing: "ease-in-out",
              fill: "forwards"
            });
      ...
      const attachHaHeaderObserver = () => {
        ...
        const scheduleHeaderLayout = () => {
          const now = Date.now();
          if (layoutScheduled || now - lastCall < THROTTLE_MS) return;
      ...
      const applyHeaderLayout = () => {
        ...
        // 5) Check visibilità (OTTIMIZZATO: evita getComputedStyle quando possibile)
        const display = haHeaderEl.style.display;
        let haHeaderVisible =
          display !== "none" &&
          (display === "flex" || display === "block" || display === "");

        // Solo se inline check non è sufficiente, usa getComputedStyle
        if (!haHeaderVisible && display !== "none") {
          try {
            haHeaderVisible =
              window.getComputedStyle(haHeaderEl).display !== "none";
          } catch (e) {
            haHeaderVisible = false; // Safe fallback
          }
        }
        ...
        if (haHeaderVisible) {
          if (haHeaderVisible !== lastHeaderVisible) {
            haHeaderHeight = Math.round(
              haHeaderEl.getBoundingClientRect().height
            );
        ...
        if (
          (topMenuMode === "push" || topMenuMode === "flip") &&
          haHeaderHeight > 0
        ) {
    ...
    if (headerWrapper) {
      const maybeSidebarWrapper = headerWrapper.querySelector(
        "#customSidebarWrapper"
      ) as HTMLElement | null;
